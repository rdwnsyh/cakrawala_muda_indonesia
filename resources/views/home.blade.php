  ...
  <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 md:py-48">
    <div class="text-center space-y-8">
      ...
      <div class="flex flex-col sm:flex-row justify-center gap-4 pt-4 animate-fadeInUp" style="animation-delay: 0.6s;">
        <a href="#our-program" class="group inline-flex items-center justify-center gap-2 bg-white/10 backdrop-blur-lg border-2 border-white/30 text-white px-8 py-4 rounded-2xl font-bold text-lg hover:bg-white hover:text-blue-700 hover:scale-105 transition-all duration-300">
      ...
      <div class="flex flex-wrap justify-center gap-8 pt-12 animate-fadeInUp" style="animation-delay: 0.8s;">
        ...
        <div class="text-center">
          <div class="text-3xl font-bold text-yellow-300">{{ $ongoingProgram->count() }}+</div>
      ...
      <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-3xl overflow-hidden shadow-2xl">
        <div class="md:flex">
          <div class="md:w-2/5 relative">
            ...
            <img src="{{ asset('storage/' . $ongoingProgramItem->poster) }}" alt="{{ $ongoingProgramItem->nama_program }}" class="w-full h-64 md:h-full object-cover">
            @else
            ...
            @endif
            <div class="absolute inset-0 bg-gradient-to-r from-blue-600/40 to-transparent md:hidden"></div>
            <div class="absolute top-4 left-4">
              <span class="px-4 py-2 bg-white/90 backdrop-blur-sm text-blue-700 font-bold rounded-full text-sm">🔥 On Going</span>
            </div>
          </div>
          ...
          <div class="md:w-3/5 p-8 md:p-12">
            <div class="flex items-center gap-2 mb-4">
              ...
              <span class="px-3 py-1 bg-yellow-400/20 text-yellow-300 text-sm font-semibold rounded-full">✨ Trending</span>
            </div>
            
            <h3 class="text-2xl md:text-3xl font-bold text-white mb-4">{{ $ongoingProgramItem->nama_program }}</h3>
            ...
            <div class="flex flex-col sm:flex-row gap-4">
              <a href="#" class="group flex-1 inline-flex items-center justify-center gap-3 bg-white text-blue-700 px-8 py-4 rounded-xl font-bold text-lg hover:shadow-2xl hover:scale-105 transition-all duration-300">
                <svg class="w-6 h-6 group-hover:rotate-12 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                </svg>
                Daftar Program
              </a>
              <a href="{{ route('programs.show', $ongoingProgramItem->slug ?? '#') }}" class="group flex-1 inline-flex items-center justify-center gap-3 bg-white/20 backdrop-blur-lg border-2 border-white/30 text-white px-8 py-4 rounded-xl font-bold text-lg hover:bg-white hover:text-blue-700 hover:scale-105 transition-all duration-300">
                <svg class="w-6 h-6 group-hover:rotate-12 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div id="our-program" class="py-20 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="relative">
            <!-- Carousel Container -->
            <div class="overflow-x-auto scroll-smooth snap-x snap-mandatory no-scrollbar cursor-grab active:cursor-grabbing" id="program-carousel">
                <div class="flex gap-6 pb-8">
                    @foreach($jenisPrograms as $jenis)
                    <div class="flex-shrink-0 snap-center w-full md:w-1/2 lg:w-1/3">
                        <div class="group bg-white rounded-2xl shadow-lg overflow-hidden h-full transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                            <div class="relative h-48 overflow-hidden">
                                @if($jenis->poster_jenis_program)
                                <img src="{{ asset('storage/' . $jenis->poster_jenis_program) }}" alt="{{ $jenis->jenis_program }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @else
                            ...
                            <div class="p-6">
                                ...
                                <div class="pt-4 border-t border-gray-100 text-center">
                                    <a href="{{ route('programs.index', ['jenis' => $jenis->jenis_program]) }}" class="group/btn inline-flex items-center gap-2 text-blue-600 font-bold hover:text-blue-700 transition-colors mx-auto">
                                        <span>Detail</span>
            ...
            <!-- Prev / Next Buttons -->
            <button onclick="document.getElementById('program-carousel').scrollBy({left: -400, behavior: 'smooth'})" class="absolute left-4 top-1/2 -translate-y-1/2 bg-white p-3 rounded-full shadow-lg hover:shadow-xl hover:scale-110 transition-all duration-300 z-10">
                <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                </svg>
            </button>
            <button onclick="document.getElementById('program-carousel').scrollBy({left: 400, behavior: 'smooth'})" class="absolute right-4 top-1/2 -translate-y-1/2 bg-white p-3 rounded-full shadow-lg hover:shadow-xl hover:scale-110 transition-all duration-300 z-10">
                <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex justify-center gap-2 mt-8">
                @php
                    $slidesCount = ceil($jenisPrograms->count() / 3);
                @endphp
                @for($i = 0; $i < $slidesCount; $i++)
                <div class="w-2 h-2 rounded-full bg-gray-300 transition-all duration-300 dot-indicator" data-index="{{ $i }}"></div>
                @endfor
            </div>
        </div>
        @else
        <div class="text-center py-12">
            <p class="text-gray-600 text-lg">Belum ada jenis program yang tersedia.</p>
        </div>
        @endif

        <!-- View All Button -->
        <div class="text-center mt-16">
            <a href="{{ route('programs.index') }}" class="group inline-flex items-center gap-3 bg-blue-600 text-white px-10 py-4 rounded-2xl font-bold text-lg hover:shadow-2xl hover:scale-105 hover:bg-blue-700 transition-all duration-300">
    ...
    <div class="fixed bottom-8 right-8 z-50">
        <a href="#"
            class="group inline-flex items-center gap-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-6 py-4 rounded-2xl font-bold shadow-2xl hover:shadow-3xl hover:scale-105 transition-all duration-300">
